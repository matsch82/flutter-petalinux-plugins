  ...
  }
  {
    auto player_handler = std::make_unique<VideoPlayerStreamHandlerImpl>(
        ...
        [texture_id, host = this]() {
          ...
          std::cerr<<"mschaff:  host->SendInitializedEventMessage("<<texture_id<<")"<<std::endl;
        },
        // OnNotifyFrameDecoded
        [texture_id, host = this]() {
          host->texture_registrar_->MarkTextureFrameAvailable(texture_id);
          std::cerr<<"mschaff:  host->texture_registrar_->MarkTextureFrameAvailable("<<texture_id<<")"<<std::endl;
        },
        // OnNotifyCompleted
        [texture_id, host = this]() {
